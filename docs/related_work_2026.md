...
## 3c. The position paper that names this paper — and indicts our vocabulary too

**[Position: Stop Anthropomorphizing Intermediate Tokens as Reasoning/Thinking Traces!](https://arxiv.org/abs/2504.09762)**
— Kambhampati, Valmeekam, Bhambri, Palod, Saldyt, Stechly, Samineni, Kalwar & Biswas.
**ICML 2026**, v4 June 2026. *(full text read directly)*

> These intermediate tokens have been called "reasoning traces" or even "thinking traces" —
> implicitly anthropomorphizing the traces … we present evidence that this
> anthropomorphization isn't a harmless metaphor, and instead is quite **dangerous** — it
> confuses the nature of these models and how to use them effectively, and **leads to
> questionable research**.

### It cites SoT by name, in the alternative-views section

> More recently, another group (Kim et al. 2026) extended the same type of LLM-based
> analysis of the intermediate tokens generated by a reasoning model — this time in terms
> of shifting voices/perspectives — and claimed that reasoning models generate *societies
> of thought*, and implied that this is what explains their effectiveness.
>
> It should be noted that these analyses are often qualitative, and **fail to establish
> direct connection between the narrative of the intermediate tokens and the final result.**

**That charge is precisely what this repository tested, quantitatively.** Their position
paper asserts the connection is unestablished; our measurements are the test:

| our result | their charge |
|---|---|
| diversity ↛ correctness at matched length (99% shrinkage) | connection not established |
| GPQA effect dissolves within-problem (+0.0023, between-problem excluded at 3.1 SE) | narrative ≠ result |
| dialogue priming = any priming by step 70 (C5) | "induce these behaviors through examples" fails |
| steering ↑ dialogic markers, MATH drift ↓ (−0.336, P=0.001) | the narrative anti-predicts |

Their §3 names the exact research pattern SoT's C5 instantiates: *"works which attribute
cognitive behaviors (like backtracking, self-verification etc.) to the models based on
their reasoning traces and try to induce these kinds of behaviors through examples in the
hope of improving the models' performance."* Our C5 found that inducing them buys speed,
not endpoint — and that the *form* induced is irrelevant.

### The "aha" argument, which generalises to "personas"

> While a human may say "aha" to indicate exactly a sudden internal state change, this
> interpretation is unwarranted for models which do not have any such internal state, and
> which on the next forward pass will only differ from the pre-aha pass by the inclusion of
> that single token in their context.

The same reasoning applies to `<persona1>` and to a segment boundary at "But actually".
A voice is not a locus of anything; it is a token in a context window.

### ⚠ It indicts our instrument, not only the paper's

We should not enjoy this. Our own segmentation is:

```python
SHIFT = re.compile(r"...(?:but|however|wait|hold on|hmm+|oh|actually|alternatively|"
                   r"another (?:idea|approach|way)|on the other hand|no,|let me reconsider...")
```

Those are exactly the phrases the paper says people notice before assuming a trace "sounds
plausible" — R1's *"hmm…", "aha!", "wait a minute"*. We built a measuring instrument out of
the anthropomorphic cue words, called its output **"perspective diversity"**, called
segments **"voices"**, and wrote sentences like *"the society is real and genuinely more
differentiated"* in a retraction.

**What survives restatement, and what does not.**

- **The measurements survive.** HSE is a well-defined statistic over cue-delimited spans and
  their embeddings. Whether the spans are "voices" or arbitrary punctuation-anchored
  windows, the finding is unchanged: *the statistic does not predict correctness once
  length and problem identity are controlled.* Nothing about the null depends on the
  metaphor.
- **The vocabulary does not survive**, and it was never load-bearing. Every result in this
  repo can be stated as: *segment-embedding dispersion over cue-delimited spans*. That is
  uglier and exactly as true.
- **One reading is sharpened.** Our "exhaust" account said dialogic markers are the *style*
  of search rather than its cause. The position paper's version is stronger and simpler:
  there may be no "search" to be the style *of* — the cue words are output statistics of a
  post-trained token distribution, and asking whether they cause accuracy may already
  concede too much.

### Action taken

Findings are being restated in non-anthropomorphic terms where the metaphor was doing
rhetorical work. `analysis/hse.py`'s docstring and the "society" language in
`docs/replication_and_tools.md` are the main offenders.
